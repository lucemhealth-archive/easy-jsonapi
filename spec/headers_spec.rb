...
describe JSONAPI::Headers do
  ...
  describe '#add' do
    ...
    it 'should add a new header if there is not already one with the same name' do
      ...
      headers.add('josh', 'demoss')
      expect(headers.get(:josh).val).to eq 'demoss'
    end

    it 'should add to the header value if there already is one with the same name' do
      expect(headers.get(:accept)).not_to be nil
      expect(headers.add(:accept, 'additional_header')).to eq 'application/vnd.api+json, additional_header'
    end
    ...
    it 'should be case insensitive and work for symbol or string for "key"' do
      ...
      expect(headers.get('josh').val).to eq 'demoss'
      headers.add(:josh, 'is')
      expect(headers.get('josh').val).to eq 'demoss, is'
      headers.add('JOSH', 'cool')
      expect(headers.get('josh').val).to eq 'demoss, is, cool'
    end
  end
  ...
  describe '#<<' do
    ...
    it 'should replace any headers with the same name' do
      ...
      expect(temp.get('ACCEPT').val).to eq 'application/vnd.api+json'
      temp << ['accept', 'changed']
      expect(temp.get('ACCEPT').val).to eq 'application/vnd.api+json, changed'
    end
  ...
  describe '#get' do
    ...
    it 'should return val the appropriate header object' do
    ...
    it 'should be case insensitive and work for symbol or string for "key"' do
      ...
      expect(temp.get(:josh).val).to eq 'demoss'
      expect(temp.get('josh').val).to eq 'demoss'
      expect(temp.get('JoSH').val).to eq 'demoss'
    end
  end
  ...
  describe '#to_s' do
    ...
    it 'should return an array of key/val hashes as a string representing Headers contents' do
  ...
  describe JSONAPI::Headers::Header do
    ...
    describe '#to_s' do
      it 'should return a Header key and val as "key => val"' do
